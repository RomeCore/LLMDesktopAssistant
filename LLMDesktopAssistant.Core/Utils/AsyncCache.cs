...
namespace LLMDesktopAssistant.Core.Utils
{
		...
		where TKey : notnull
	{
		...
		/// Disposes item if it implements <see cref="IDisposable"/> or <see cref="IAsyncDisposable"/>.
		/// </summary>
		/// <param name="key">The key of the item to remove.</param>
		/// <returns><see langword="true"/> if the item was found and removed; otherwise, <see langword="false"/>.</returns>
		public bool TryRemove(TKey key)
		{
			var removed = _storage.TryRemove(key, out var removedItem);
		...
		/// <summary>
		/// Removes an item from the cache if it exists.
		/// Disposes item if it implements <see cref="IDisposable"/> or <see cref="IAsyncDisposable"/> (and awaits for disposing).
		/// </summary>
		/// <param name="key">The key of the item to remove.</param>
		/// <returns><see langword="true"/> if the item was found and removed; otherwise, <see langword="false"/>.</returns>
		...
		/// <summary>
		/// Removes all items that have exceeded their sliding expiration time.
		/// Awaits for all items to be disposed if they implement <see cref="IAsyncDisposable"/>.
		/// </summary>

...
namespace LLMDesktopAssistant.Desktop.Execution.Terminals
{
	...
	/// user input to an external <see cref="IPtyConnection"/>.
	/// The view does not manage the process lifecycle: the PTY is created, read and killed
	/// by the <see cref="ProcessLauncher"/>. The view only renders the terminal buffer and
	/// writes user input to the PTY, so it can be detached and recreated without affecting
	/// the running process.
	/// </summary>
	public class TerminalView : Control
	{
		private XT.Terminal? _terminal;
		private FormattedText _measureText = null!;
		...
		// Process state (owned by the launcher, mirrored here for input routing only)
		...
		// Cursor blinking
		private DispatcherTimer _cursorBlinkTimer = null!;
		...
		public static readonly DirectProperty<TerminalView, XT.Terminal?> TerminalProperty =
			AvaloniaProperty.RegisterDirect<TerminalView, XT.Terminal?>(
				nameof(Terminal),
				o => o.Terminal,
				(o, v) => o.Terminal = v);

		public static readonly DirectProperty<TerminalView, IPtyConnection?> PtyConnectionProperty =
			AvaloniaProperty.RegisterDirect<TerminalView, IPtyConnection?>(
				nameof(PtyConnection),
				o => o.PtyConnection,
				(o, v) => o.PtyConnection = v);
		...
		protected override void OnInitialized()
		{
			...
			// If no terminal has been assigned yet, create a default one so the view
			// can render even before the session is attached.
			if (_terminal == null)
			{
				...
				SubscribeTerminalEvents(_terminal);
			}
		...
		private void SubscribeTerminalEvents(XT.Terminal terminal)
		{
		...
		private void UnsubscribeTerminalEvents(XT.Terminal terminal)
		{
			terminal.DataReceived -= OnTerminalDataReceived;
			terminal.BufferChanged -= OnTerminalBufferChanged;
			terminal.CursorStyleChanged -= OnTerminalCursorStyleChanged;
		...
		private void ApplyStyleOptions(XT.Options.TerminalOptions options)
		{
		...
		{
			get => _terminal?.Buffer.ViewportY ?? 0;
			set
			{
				if (_terminal == null)
					return;
		...
		public int ViewportLines => _terminal?.Rows ?? 0;

		/// <summary>
		/// Gets or sets the terminal emulation instance rendered by this view.
		/// Assigning a new terminal re-subscribes all terminal events.
		/// </summary>
		public XT.Terminal? Terminal
		{
			get => _terminal;
			set
			{
				...
				if (_terminal != null)
					UnsubscribeTerminalEvents(_terminal);
				...
				{
					SubscribeTerminalEvents(_terminal);
					ApplyStyleOptions(_terminal.Options);
				}
		...
		/// <summary>
		/// Gets or sets the PTY connection that the view writes user input to.
		/// The view never reads from the connection — the launcher does.
		/// </summary>
		public IPtyConnection? PtyConnection
		{
			get => _ptyConnection;
			set
			{
				...
				if (_ptyConnection != null)
				{
					...
					// The process might have exited before the subscription — check for it.
					if (_ptyConnection.WaitForExit(0))
		...
		public async Task PasteAsync()
		{
			...
			if (!string.IsNullOrEmpty(text))
			{
				...
				if (_terminal?.BracketedPasteMode == true)
				{
					text = $"\u001b[200~{text}\u001b[201~";
				}
		...
		{
			if (_terminal == null || !_terminal.Selection.HasSelection)
				return false;
		...
		{
			// The process is launched by the ProcessLauncher — nothing to start here.
		...
		protected override void OnDetachedFromLogicalTree(LogicalTreeAttachmentEventArgs e)
		{
			...
			// The PTY process is owned by the launcher and must survive view detachment,
			// so we only unsubscribe from terminal events here.
			if (_terminal != null)
				UnsubscribeTerminalEvents(_terminal);
		}
		...
		private void OnCursorBlinkTick(object? sender, EventArgs e)
		{
			if (CursorBlink && IsFocused)
			{
				...
				for (int y = 0; y < (_terminal?.Rows ?? 0); y++)
				{
		...
		protected override async void OnKeyUp(KeyEventArgs e)
		{
			...
			// Capture the connection reference locally
			var ptyConnection = _ptyConnection;
			if (ptyConnection == null || _processExitHandled != 0 || _terminal == null)
			{
				base.OnKeyUp(e);
				return;
			}
		...
		protected override async void OnTextInput(TextInputEventArgs e)
		{
			...
			// Capture the connection reference locally
			var ptyConnection = _ptyConnection;
			if (ptyConnection == null || _terminal == null || string.IsNullOrEmpty(e.Text) || _processExitHandled != 0)
			{
		...
		protected override async void OnPointerPressed(PointerPressedEventArgs e)
		{
			...
			try
			{
				...
				if (_ptyConnection == null || _terminal == null)
					return;
		...
		protected override async void OnPointerWheelChanged(PointerWheelEventArgs e)
		{
			...
			if (_ptyConnection != null && _terminal != null && _terminal.MouseTrackingMode != XT.Input.MouseTrackingMode.None)
			{
				var point = e.GetPosition(this);
				var col = (int)(point.X / _charWidth);
				var row = (int)(point.Y / _charHeight);
		...
		protected override async void OnGotFocus(FocusChangedEventArgs e)
		{
			...
			if (_ptyConnection != null && _terminal != null && _terminal.SendFocusEvents)
			{
		...
		private void RenderPreeditText(DrawingContext context, int viewportY, double scale)
		{
			...
			if (string.IsNullOrEmpty(preeditText) || _terminal == null)
				return;

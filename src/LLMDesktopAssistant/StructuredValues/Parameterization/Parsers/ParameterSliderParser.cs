...
namespace LLMDesktopAssistant.StructuredValues.Parameterization.Parsers
{
	...
	public class ParameterSliderParser : IParameterSchemaParser
	{
			...
			ParameterSchemaLimitationType requestedType, ParameterSchemaPath path, AppendOnlyList<ParameterSchemaParsingError> errors)
		{
			...
			if (ParameterSchemaParserHelpers.RequireNumber(schema, "min", path, errors, out var min) is { } errMin)
				return errMin;
			if (ParameterSchemaParserHelpers.RequireNumber(schema, "max", path, errors, out var max) is { } errMax)
				return errMax;

			double step = isInteger ? 1 : 0;
			if (ParameterSchemaParserHelpers.OptionalNumber(schema, "step", path, errors, out var stepOpt) is { } errStep)
				return errStep;
			if (stepOpt.HasValue)
				step = stepOpt.Value;

			var def = min;
			if (ParameterSchemaParserHelpers.OptionalNumber(schema, "default", path, errors, out var defOpt) is { } errDef)
				return errDef;
			if (defOpt.HasValue)
				def = defOpt.Value;
